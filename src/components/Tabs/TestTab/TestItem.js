...
const TestItem = ({id, ionicCode, test, updatedTime, addedBy }) => {
  ...
  const formatDate = (dateString) => {
    ...
    return date.toLocaleString('en-US', {
      month: 'short',  // Displays the short month name
      day: '2-digit',  // Displays the day with 2 digits
      year: '2-digit', // Displays the year with 2 digits
      hour: '2-digit', // Displays the hour in 2 digits
      minute: '2-digit', // Displays the minute in 2 digits
  ...
  const deleteTest = async (code) => {
    try {
      const response = await authDelete(
        `test/${patientTimeLineID}/${code}`,
        user.token
      );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.detailRow}>
        ...
        <Text style={styles.time}>
          Time: <Text style={styles.semiBoldText}>{formattedDate}</Text>
        </Text>
      </View>
