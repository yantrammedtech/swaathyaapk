...
const MedicineForm = ({ visible, onClose }) => {
    ...
    const handleDosageIncrease = () => {
        setMedicineData((prev) => {
          const newData = [...prev];
          const currentValue = newData[index]?.doseCount || 0;
          const newValue = currentValue + 1;
    
          if (newValue > 999) { // Example constraint: maximum 999
      ...
      const handleDosageDecrease = () => {
        setMedicineData((prev) => {
          const newData = [...prev];
          const currentValue = newData[index]?.doseCount || 0;
          const newValue = Math.max(currentValue - 1, 0); // Ensure the value doesn't go below 0
          ...
          if (!finalData[0] || finalData[0].medicationTime.split(',').length > (finalData[0].Frequency ?? 0)) {
            if(!finalData[0].medicationTime.includes("As Per Need")){
              Toast.show({
                type: 'error',
                text1: 'Error',
                text2: `Please Select ${finalData[0].Frequency} Time of Medication`,
                position: 'top',
                visibilityTime: 4000,
              });
              return;
  ...
  const [dropdownVisible, setDropdownVisible] = useState(false);
    ...
    const fetchMedicineList = async (text, index) => {
      ...
      const selectMedicine = (medicineName, index) => {
        // Set the selected medicine name for the corresponding index
        setMedicineData((prev) => {
          const newData = [...prev];
