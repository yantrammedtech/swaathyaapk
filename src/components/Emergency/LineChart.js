  ...
  const barData = async () => {
    ...
    try {
      const barGraphResponse = await authFetch(
        `patient/${user.hospitalID}/patients/count/fullYearFilter/${patientStatus.emergency}?filter=${filterMonth}&filterValue=${filterValue}&zone=${zoneType}`,
        user.token
      );
      console.log("barGraphResponse=yt==",barGraphResponse)
