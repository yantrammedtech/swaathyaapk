...
const TransferPatientForm = () => {
  ...
  const validateInputs = (changedField, newValue) => {
    ...
    if (
      changedField === "temp" &&
      (temperatureValue < 20 || temperatureValue > 45)
    ) {
    ...
    if (changedField === "bpH" && bpHighValue) {
      if (bpHighValue > 200 || bpHighValue < 50) {
        newErrors.bpH = `BP High should be between ${
          bpLowValue || 50
        } and 200 mm Hg.`;
      } else if (bpHighValue < bpLowValue) {
        newErrors.bpH =
          "Blood Pressure High cannot be less than Blood Pressure Low.";
      }
    }
    ...
    if (changedField === "bpL" && bpLowValue) {
      if (bpLowValue < 30 || bpLowValue > 200) {
        ...
        newErrors.bpL =
          "Blood Pressure Low cannot be greater than Blood Pressure High.";
      }
    }
    ...
    if (
      changedField === "respiratoryRate" &&
      (respiratoryRateValue < 1 || respiratoryRateValue > 40)
    ) {
      newErrors.respiratoryRate =
        "Respiratory Rate should be between 1 and 40 breaths per minute.";
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.transferType.value) {
      Toast.show({
        ...
        text2: "Transfer Type is required",
        visibilityTime: 1500,
      });
      return;
    }

    if (formData.transferType.value === transferTypeValue.internal) {
      // Object.keys(formData).forEach((el) => {
        for (const el of Object.keys(formData)) {
        if (!formData[el].valid && el == "wardID" && el == "departmentID") {
        ...
        if (
          ...
          Alert.alert("Please Select Ward");

          Toast.show({
            type: "error",
            text1: "Validation Error",
            text2: "Please Select Ward",
            visibilityTime: 1000,
          });
          return;
        }
        if (
          ...
          Alert.alert("Please Select Doctor");
          Toast.show({
            type: "error",
            text1: "Validation Error",
            text2: "Please Select Doctor",
            visibilityTime: 3000,
          });
          return;
        }
      ...
      Object.keys(formData).forEach((el) => {
        ...
        if (
          ...
          Alert.alert("Please Enter Relative Name");
          Toast.show({
            type: "error",
            text1: "Validation Error",
            text2: "Please Enter Relative Name",
            visibilityTime: 1500,
          });
          return;
        }
        if (
          ...
          Toast.show({
            type: "error",
            text1: "Validation Error",
            text2: "Please Enter Hospital Name",
            visibilityTime: 1500,
          });
          return;
        }
  ...
  <RadioButton.Group
      ...
      >
        <View style={styles.radioGroup}>
          <RadioButton.Item
            label="Internal"
            value={transferTypeValue.internal}
          />
          <RadioButton.Item
            label="External"
            value={transferTypeValue.external}
          />
        </View>
      </RadioButton.Group>
      {formData.transferType.showError && (
        <Text style={{ color: "red" }}>{formData.transferType.message}</Text>
      )}
       </>
)}
            ...
            <View style={styles.textInput}>
              <Picker
                ...
                onValueChange={(value) => {
                  setDoctor(value);
                  const departmentID =
                    doctorList.find((el) => el.id === Number(value))
                      ?.departmentID || 0;
                  setFormData((prevData) => ({
                    ...prevData,
                    userID: {
      ...
      <TextInput
        ...
        style={styles.input}
      />
      {formData?.oxygen?.showError && (
        <Text style={styles.errorText}>{formData.oxygen.message}</Text>
      )}
      ...
      <View style={styles.row}>
        <View style={styles.halfInputContainer}>
          ...
          />
          {formData?.bpH?.showError && (
            <Text style={styles.errorText}>{formData.bpH.message}</Text>
          )}
        </View>
        <View style={styles.halfInputContainer}>
          <TextInput
            ...
            style={styles.halfInput}
          />
          {formData?.bpL?.showError && (
            <Text style={styles.errorText}>{formData.bpL.message}</Text>
          )}
        </View>
      </View>
      <View style={styles.row}>
        <View style={styles.halfInputContainer}>
          <TextInput
            ...
            style={styles.halfInput}
          />
          {formData?.temp?.showError && (
            <Text style={styles.errorText}>{formData.temp.message}</Text>
          )}
        </View>
        <View style={styles.halfInputContainer}>
          <TextInput
            ...
            style={styles.halfInput}
          />
          {formData?.pulse?.showError && (
            <Text style={styles.errorText}>{formData.pulse.message}</Text>
          )}
        </View>
      </View>
      ...
      <TextInput
        ...
        style={styles.input}
      />
      {formData?.respiratoryRate?.showError && (
        <Text style={styles.errorText}>{formData.respiratoryRate.message}</Text>
      )}
      ...
      <TextInput
        ...
        style={styles.input}
      />
      {formData?.relativeName?.showError && (
        <Text style={styles.errorText}>{formData.relativeName.message}</Text>
      )}
